...
const fetchData = debounce(async () => {
  ...
  try {
    const res = await fetch(`http://${url}/partial_name/${searchTerm}`)
...
<div class="bg-neutral-800 p-2 shadow-lg">
  <div class="container mx-auto grid" style="grid-template-columns: 1fr 3fr 1fr;">
    ...
    <div>
      <label class="relative block">
        ...
        <input class="placeholder:text-zinc-400 block bg-inputText-color  w-full border-2 border-inputText-color text-font-color rounded-md py-2 pl-9 pr-3 shadow-sm focus:outline-none focus:border-main-color" placeholder="Search for anything..." type="text" name="search" bind:value={searchTerm} on:input={fetchData} on:focus={onFocus} on:blur={lostFocus} on:resize={handleResize} />
      </label>
          ...
          {#each dataJson as suggestion}
            <li class="lg:w-96 md:w-80 w-72 py-1 px-2  hover:cursor-pointer text-font-color hover:text-main-color ease-out duration-100">
              <a href={`/anime/${suggestion.anime_id}`} class="flex items-center">
                <img style="width: 32px; height:45px;" src={`http://${url}/image/${suggestion.anime_id}`} alt={suggestion.name} draggable="false">
    ...
    <div><Github color="#efef" width={25} height={25}/></div>
  </div>
</div>
